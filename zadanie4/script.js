...
var N = parseInt(prompt("Podaj liczbe dyskow:"));
var N_points = parseInt(prompt("Ile punktow przyciagania chcesz dodac?"));
...
function drawDisc(disc){
    ...
    for(let i=0; i<N_points; i++){
        let delta_x = Math.round(disc.x - points[i].x);
        let delta_y = Math.round(disc.y - points[i].y);
...
for(let i=0; i<N_points; i++)
{
    let x = parseInt(prompt('Podaj wspolrzedna x dla '+ (i+1) + ' punktu. [0, 1023]'));
    let y = parseInt(prompt('Podaj wspolrzedna y dla '+ (i+1) + ' punktu. [0, 767]'));
